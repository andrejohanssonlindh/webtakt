...
| Keybinds | `settings.keybinds.{play,record,stopAll,manual,hold,arp,octaveUp,octaveDown,fx1..fx4}` — each an `event.code`. Click a keybind button then press a key to rebind (Escape cancels; a clash auto-unbinds the previous owner with a warning). Defaults: Space / Enter / Backspace / M / X / Z / ↑ / ↓ / C V B N. `octaveUp/Down` call `Keyboard.shiftOctave(±1)` (mirrors the OCT± buttons). Read by index.html's transport keydown handler. |
