...
| Setting | Drives |
|---|---|
| Synced-knob grid | `settings.bpmGrid` (1/32 / 1/64 / 1/128). The **finest** division the BPM-synced knobs step to / snap to / can reach. Calls `BpmSync.setSnapResolution(gridBase)` which (a) rebuilds the live `MUSICAL_SNAP_32` array with extra sub-1/32 snap targets and (b) sets the free-drag quantize step (`quantizeCount`) so each free-drag step is one grid unit at the chosen resolution (1/32 → 1, 1/64 → 0.5, 1/128 → 0.25 stored units). **Stored counts stay in 1/32 units** (`GRID_BASE` is fixed at 32) so no saved project is ever rescaled — a 1/64 note is the fractional count 0.5. FX sync knobs' `bpmMin` was lowered to 0.25 so the finer snaps are reachable when enabled. |
...
| Capture Play key | `settings.capturePlay` (checkbox, default **on**). When on, the Play/Stop keydown handler blurs `document.activeElement` before clicking PLAY, so pressing the play key while a button has focus never also re-activates that button (native Space-activates-focused-element double-fire). Off = native behaviour. |
| CapsLock = lowercase | `settings.capsNormalizeKeys` (checkbox, default **on**). `Keyboard._normKey` folds a CapsLock-uppercased single letter (`"A"`→`"a"`) before keymap lookup so the piano keeps working with CapsLock on. Off = case-sensitive, so an uppercase letter can be bound to a different function. |
| Keybinds | `settings.keybinds.{play,record,stopAll,manual,hold,arp,octaveUp,octaveDown,moveLeft,moveRight,fx1..fx4}` — each an `event.code`. Click a keybind button then press a key to rebind (Escape cancels; a clash auto-unbinds the previous owner with a warning). Defaults: Space / Enter / Backspace / M / X / Z / ↑ / ↓ / ← / → / C V B N. `octaveUp/Down` call `Keyboard.shiftOctave(±1)` (mirrors the OCT± buttons); `moveLeft/Right` shift one step — moving the selected trigger, or rotating the whole pattern when no step is selected. Read by index.html's transport keydown handler. |
| Track number keys (fixed) | Not rebindable; shown as a reference note in the pane. Digits **1–N**: bare = mute/unmute that track; **Shift+1–N** = `state.selectTrack` (switch to that track); **Alt+1–N** = `_goToPage` (jump to that pattern page). In **drum mode**, bare digits finger-drum instead — Shift/Alt+digit still select/page (Keyboard.js ignores digit-drum when a modifier is held). |
