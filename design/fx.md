...
| AutoPan | `AutoPanFX.js` | `pan.depth` / `pan.shape` (0=pan↔1=tremolo) / `pan.sync` (Hz↔BPM rate). One sine LFO → a `StereoPanner.pan` (weighted 1−shape) and a tremolo `GainNode` (weighted shape); tremolo rests below unity so the LFO lifts up, not clips. `shape` is js-driven (composite) but **whitelisted** in `TRACK_JS_LFO_PARAMS`, so it IS an LFO target via the continuous-JS driver. |
...
| Duck | `DuckFX.js` | `duck.depth` (0–1) + `duck.attack` / `duck.hold` / `duck.release`, each a **unified MS↔BPM sync knob** (`type:'sync'`, click-centre toggle; ms value + `*SyncMode` + `*Count32` 1/32-grid, same model as DelayFX). **Trigger-driven** sidechain ducker (no rhythmic source of its own): `trigger(time)` dips the output gain to `1 − depth` over attack, holds, then recovers over release. Used on the global FX track, pulsed by the kick via the follow loop (`Sequencer._fireStep → Track.triggerDuck`) — the Syntakt kick-pump as a block instead of a negative-ADH amp. `depth` is continuous (p-lock / JS-LFO via `TRACK_JS_LFO_PARAMS`); the stage times are read at trigger time (track-level, BPM-synced via `setBpm`). See `design/audio-signal-chain.md` → Global FX Track. |
...
**LFO targeting & js-driven params.** The LFO normally connects to **AudioParams directly**. A `modulatable` FX param with no `resolveAudioParam` (a *composite* / js-driven one) would be a dead AudioParam target, so `getLFOAssignableParams()` filters it out of the LFO dropdown — **unless** it is whitelisted in `TRACK_JS_LFO_PARAMS` (`Track.js`). Whitelisted continuous-JS params (`comb.freq`, `gate.depth`/`smooth`, `tape.wow`/`spread`, `pan.shape`, plus the machine params `op*.ratio`, `color`, `noise.color`/`amount`, `hum`, `spread`, `ensemble`) are promoted to real LFO targets: the `Track._jsLfoTick` rAF driver sweeps them via `setParam(path, base + lfo.getCurrentValue())`, so they CAN modulate a held/ringing note. Non-whitelisted composite params (e.g. the hidden `*.bpmCount32` divisions) stay mod/CC-only. Inclusion requires the param be *continuous* (its `setParam` takes effect between note triggers) — params only read at note-on are excluded. Full mechanics + the slot-0 machine caveat: `js/signal/LFO.md` → *Continuous JS-only destinations*.
