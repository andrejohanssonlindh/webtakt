/**
 * machines/cymbal_analogue.js — AnalogueCymbalMachine tests
 *
 * Mirrors cymbal.js: shares Cymbal's six-oscillator cluster → HPF → BP topology,
 * so audibility + high-band dominance checks apply.
 */
...
suite('AnalogueCymbalMachine', () => {

  test('produces audible output', async () => {
    const { track, ctx, sampleRate } = await makeOfflineTrack('cymbal.analogue', DURATION);
