 ...
 *   'delay.bpmCount32' — integer count of 1/32 notes, default 8 (=1/4) (used when syncMode='bpm')
...
export class DelayFX {
  ...
  _applyBpmTime() {
    const secs = count32ToSeconds(this._params['delay.bpmCount32'], this._bpm);
  ...
  getParamList() {
    return [
      // Unified MS/BPM sync knob. FXPanel renders one knob + an MS/BPM toggle;
      // in BPM mode the knob sweeps the 1/32 grid (msPath/bpmPath select which
      // underlying param the knob writes). Both underlying params stay listed
      // (hidden) so p-lock & LFO machinery keep working unchanged.
      {
        path: 'delay.sync', label: 'Time', type: 'sync',
      ...
      // an LFO sweeping the full range pitch-shifts/flanges the delay line and
      // self-oscillates with feedback up. A modest window keeps it musical.
      { path: 'delay.time',       label: 'Time',     type: 'number', min: 0.001, max: 2.0,  default: 0.375, modulatable: true, lfoMin: 0.02, lfoMax: 0.6, plockMode: 'audioParam', hidden: true },
      { path: 'delay.bpmCount32', label: 'Division', type: 'number', min: 1,     max: 64,   default: 8,     modulatable: true, plockMode: 'js', hidden: true },
  ...
  fromJSON(obj) {
    ...
    // Back-compat: legacy projects stored a 'delay.bpmDiv' division string.
    if (params['delay.bpmDiv'] !== undefined && params['delay.bpmCount32'] === undefined) {
      params['delay.bpmCount32'] = divToCount32(params['delay.bpmDiv']);
    }
    delete params['delay.bpmDiv'];
