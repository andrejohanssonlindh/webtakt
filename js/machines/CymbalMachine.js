...
export class CymbalMachine extends Machine {
  ...
  noteOn(midiNote, velocity, time) {
    ...
    // (C4 = the resting `tune`). Mirrors the `tune.apply` cluster spread; holds
    // until the next note or a `tune`/LFO write.
    ...
    if (ratio !== 1) {
      ...
      this._oscs.forEach((osc, i) => {
        ...
        osc.frequency.setValueAtTime(tune * RATIOS[i] * ratio, t);
      });
    }
